...
MODEL_PATH = MODEL_DIR / "local_outlier_factor" / "models" / "local_outlier_factor_creditcard.pkl"
...
def evaluate_model(model_path, test_path, model_name, reports_dir, images_dir, predictions_path, report_path):
    ...
    if not expected_features:
        expected_features = [
            col
            for col in df.columns
            if col not in LABEL_COLUMN_CANDIDATES and col not in {"label", "target", "class", "Class", "y"}
        ]
    ...
    save_amount_vs_score_plot(df, anomaly_scores, images_dir / "amount_vs_anomaly_score.png")
    ...
    report = {
        ...
        "plots": {
            ...
            "amount_vs_anomaly_score": str(images_dir / "amount_vs_anomaly_score.png"),
        },
    }
