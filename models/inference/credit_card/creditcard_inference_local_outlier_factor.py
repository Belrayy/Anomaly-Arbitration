...
MODEL_PATH = PROJECT_ROOT / "models" / "training" / "local_outlier_factor" / "models" / "local_outlier_factor_creditcard.pkl"
INPUT_FILE = PROJECT_ROOT / "datasets" / "data" / "split" / "creditcard_inference.csv"
OUTPUT_FILE = BASE_DIR / "predictions" /"predictions_creditcard_local_outlier_factor.json"
...
def infer(model_path, input_file, output_file):
    ...
    if not feature_columns:
        feature_columns = [col for col in df.columns if col not in {"Class", "class", "label", "target", "y"}]
