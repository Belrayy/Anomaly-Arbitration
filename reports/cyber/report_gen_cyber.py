...
attack_count = int(pred_distribution.get("1", 0)) if "1" in pred_distribution else int(np.sum(predictions_arr == 1))
benign_count = n - attack_count
attack_pct = (attack_count / n * 100) if n > 0 else 0.0
benign_pct = (benign_count / n * 100) if n > 0 else 0.0

lower_fence = q1 - 1.5 * iqr_val
upper_fence = q3 + 1.5 * iqr_val
outliers = [
    (fid, pred, sc)
    for fid, pred, sc in zip(flow_ids, predictions, scores)
    if sc < lower_fence or sc > upper_fence
]

top_anomalies = sorted(
    zip(flow_ids, predictions, scores), key=lambda x: x[2]
)[:20]
...
plt.title("Distribution of Network Flow Anomaly Scores")
...
labels = [
    f"Benign (0)\n{benign_count} ({benign_pct:.1f}%)",
    f"Attack (1)\n{attack_count} ({attack_pct:.1f}%)",
]
...
plt.title("Benign/Attack Classification Distribution")
plt.tight_layout()
...
plt.title("Boxplot of Network Flow Anomaly Scores")
plt.xlabel("Anomaly Score")
plt.yticks([])
plt.tight_layout()
...
latex_lines.append(r"\definecolor{rowattack}{HTML}{F5B7B1}")
latex_lines.append(r"\definecolor{rowmild}{HTML}{FDEBD0}")
latex_lines.append(r"\definecolor{rowalt}{HTML}{F2F2F2}")
latex_lines.append("")
latex_lines.append(r"\hypersetup{colorlinks=true,linkcolor=primaryblue,urlcolor=primaryblue,citecolor=primaryblue,pdftitle={Network Intrusion Report},pdfauthor={Anomaly Arbitration}}")
latex_lines.append("")
...
latex_lines.append(r"\fancyhead[L]{\small\textcolor{primaryblue}{Network Intrusion Report}}")
...
latex_lines.append(r"{\Huge\bfseries\color{primaryblue} Network\\[0.3em] Intrusion Detection Report by Anomaly Arbitration\par}")
latex_lines.append(r"\vspace{1.5cm}")
latex_lines.append(r"{\large Statistical Analysis of Network Flow Anomaly Predictions on the CIC-IDS2017 Dataset\par}")
...
latex_lines.append(r"{\normalsize Total Network Flows Analyzed: " + f"{n_total}" + r"\par}")
latex_lines.append(r"\vfill")
latex_lines.append(r"{\small Network Security Monitoring - Anomaly Arbitration\par}")
...
latex_lines.append(
    ...
    f"generated from the CIC-IDS2017 dataset, which contains labeled network traffic flows capturing "
    f"both benign user activity and a range of simulated attack scenarios (including DoS, DDoS, "
    f"brute force, infiltration, botnet, and web attack traffic). "
    f"The dataset contains {benign_count} flows classified as benign ({benign_pct:.2f}\\%) and "
    f"{attack_count} flows classified as attack traffic ({attack_pct:.2f}\\%). "
...
latex_lines.append(
    "The statistics below summarize the distribution of model-generated anomaly scores across all "
    "network flows in the CIC-IDS2017 prediction set. Lower scores indicate a stronger deviation "
    "from normal (benign) traffic behavior."
)
latex_lines.append(r"\begin{table}[H]")
latex_lines.append(r"\centering")
latex_lines.append(r"\caption{Descriptive Statistics of Network Flow Anomaly Scores}")
...
latex_lines.append(r"\subsection{Benign/Attack Classification Distribution}")
latex_lines.append(r"\begin{table}[H]")
latex_lines.append(r"\centering")
...
latex_lines.append(r"\midrule")
latex_lines.append(f"Benign (0) & {benign_count} & {benign_pct:.2f}\\% \\\\")
latex_lines.append(r"\rowcolor{rowalt}" + f"Attack (1) & {attack_count} & {attack_pct:.2f}\\% \\\\")
latex_lines.append(r"\bottomrule")
latex_lines.append(r"\end{tabular}")
latex_lines.append(r"\end{table}")
latex_lines.append("")
...
latex_lines.append(
    ...
    f"mean anomaly score of the underlying network traffic population lies within this interval."
)
latex_lines.append("")
...
latex_lines.append(r"\caption{Histogram of network flow anomaly scores with mean and median indicators}")
...
latex_lines.append(r"\caption{Proportion of benign vs. attack network flows}")
latex_lines.append(r"\end{figure}")
latex_lines.append("")
...
latex_lines.append(r"\caption{Boxplot showing quartiles and outliers of network flow anomaly scores}")
latex_lines.append(r"\end{figure}")
latex_lines.append("")

latex_lines.append(r"\section{Top Anomalous Flows}")
latex_lines.append(
    f"The table below lists the top {len(top_anomalies)} network flows with the lowest anomaly scores "
    f"(most anomalous / most likely to represent malicious traffic), color-coded by severity relative "
    f"to the IQR fences (lower fence = {fmt(lower_fence)}, upper fence = {fmt(upper_fence)})."
)
latex_lines.append(r"\begin{longtable}{rrr}")
latex_lines.append(r"\caption{Top Anomalous Flows by Score} \label{tab:topanomalies} \\")
latex_lines.append(r"\toprule")
latex_lines.append(r"\rowcolor{primaryblue!15}\textbf{Flow ID} & \textbf{Prediction} & \textbf{Anomaly Score} \\")
latex_lines.append(r"\midrule")
...
for fid, pred, sc in top_anomalies:
    color = anomaly_row_color(sc)
    latex_lines.append(f"\\rowcolor{{{color}}}{fid} & {pred} & {fmt(sc)} \\\\")
