...
fail_count = int(pred_distribution.get("1", 0)) if "1" in pred_distribution else int(np.sum(predictions_arr == 1))
pass_count = n - fail_count
fail_pct = (fail_count / n * 100) if n > 0 else 0.0
pass_pct = (pass_count / n * 100) if n > 0 else 0.0

lower_fence = q1 - 1.5 * iqr_val
upper_fence = q3 + 1.5 * iqr_val
outliers = [
    (uid, pred, sc)
    for uid, pred, sc in zip(unit_ids, predictions, scores)
    if sc < lower_fence or sc > upper_fence
]

top_anomalies = sorted(
    zip(unit_ids, predictions, scores), key=lambda x: x[2]
)[:20]
...
plt.title("Distribution of Sensor Anomaly Scores")
...
plt.title("Boxplot of Sensor Anomaly Scores")
plt.xlabel("Anomaly Score")
plt.yticks([])
plt.tight_layout()
...
latex_lines.append(r"\definecolor{rowfail}{HTML}{F5B7B1}")
latex_lines.append(r"\definecolor{rowmild}{HTML}{FDEBD0}")
latex_lines.append(r"\definecolor{rowalt}{HTML}{F2F2F2}")
latex_lines.append("")
latex_lines.append(r"\hypersetup{colorlinks=true,linkcolor=primaryblue,urlcolor=primaryblue,citecolor=primaryblue,pdftitle={SECOM Manufacturing Anomaly Report},pdfauthor={Anomaly Arbitration}}")
latex_lines.append("")
...
latex_lines.append(r"\fancyhead[L]{\small\textcolor{primaryblue}{SECOM Manufacturing Anomaly Report by Anomaly Arbitration}}")
...
latex_lines.append(r"{\Huge\bfseries\color{primaryblue} SECOM Semiconductor\\[0.3em] Manufacturing Anomaly Report\par}")
latex_lines.append(r"\vspace{1.5cm}")
latex_lines.append(r"{\large Statistical Analysis of Wafer Sensor Anomaly Predictions on the UCI SECOM Dataset\par}")
...
latex_lines.append(r"{\normalsize Total Production Units Analyzed: " + f"{n_total}" + r"\par}")
latex_lines.append(r"\vfill")
latex_lines.append(r"{\small SECOM Process Monitoring - Anomaly Arbitration\par}")
...
latex_lines.append(
    ...
    f"generated from the UCI SECOM dataset, which contains sensor readings collected from a "
    f"semiconductor manufacturing process. Each record corresponds to a production unit measured "
    f"across a large array of process sensors during fabrication. "
    f"The dataset contains {pass_count} units classified as pass / in-control ({pass_pct:.2f}\\%) and "
    f"{fail_count} units classified as fail / anomalous ({fail_pct:.2f}\\%). "
...
latex_lines.append(
    ...
    "production units in the SECOM prediction set. Lower (more negative) scores indicate a stronger "
    "deviation from normal process behavior."
)
latex_lines.append(r"\begin{table}[H]")
latex_lines.append(r"\centering")
latex_lines.append(r"\caption{Descriptive Statistics of Sensor Anomaly Scores}")
...
latex_lines.append(r"\subsection{Pass/Fail Classification Distribution}")
latex_lines.append(r"\begin{table}[H]")
latex_lines.append(r"\centering")
...
latex_lines.append(r"\midrule")
latex_lines.append(f"Pass (0) & {pass_count} & {pass_pct:.2f}\\% \\\\")
latex_lines.append(r"\rowcolor{rowalt}" + f"Fail (1) & {fail_count} & {fail_pct:.2f}\\% \\\\")
latex_lines.append(r"\bottomrule")
latex_lines.append(r"\end{tabular}")
latex_lines.append(r"\end{table}")
latex_lines.append("")
...
latex_lines.append(
    ...
    f"mean anomaly score of the underlying SECOM production process lies within this interval."
)
latex_lines.append("")
...
latex_lines.append(r"\caption{Histogram of sensor anomaly scores with mean and median indicators}")
...
latex_lines.append(r"\caption{Proportion of pass vs. fail production units}")
latex_lines.append(r"\end{figure}")
latex_lines.append("")
...
latex_lines.append(r"\caption{Boxplot showing quartiles and outliers of sensor anomaly scores}")
latex_lines.append(r"\end{figure}")
latex_lines.append("")

latex_lines.append(r"\section{Top Anomalous Units}")
latex_lines.append(
    f"The table below lists the top {len(top_anomalies)} production units with the lowest anomaly scores "
    f"(most anomalous / most likely to indicate a process fault), color-coded by severity relative to "
    f"the IQR fences (lower fence = {fmt(lower_fence)}, upper fence = {fmt(upper_fence)})."
)
latex_lines.append(r"\begin{longtable}{rrr}")
latex_lines.append(r"\caption{Top Anomalous Units by Score} \label{tab:topanomalies} \\")
latex_lines.append(r"\toprule")
latex_lines.append(r"\rowcolor{primaryblue!15}\textbf{Unit ID} & \textbf{Prediction} & \textbf{Anomaly Score} \\")
latex_lines.append(r"\midrule")
...
for uid, pred, sc in top_anomalies:
    color = anomaly_row_color(sc)
    latex_lines.append(f"\\rowcolor{{{color}}}{uid} & {pred} & {fmt(sc)} \\\\")
